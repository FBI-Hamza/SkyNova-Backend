...
exports.createNonVerbalQuizResult = async (req, res, next) => {
  try {
    const { quizId, answers, marks, totalMarks } = req.body;
...
exports.deleteNonVerbalQuizResult = async (req, res, next) => {
  try {
    ...
    const deletedResult = await nonVerbalQuizResult.findByIdAndDelete(resultId);

    if (!deletedResult) {
      return res.status(404).json({ message: "Non Verbal Quiz result not found" });
    }
