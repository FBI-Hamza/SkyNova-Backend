...
exports.createNonVerbalQuestion = async (req, res) => {
  try {
    ...
    const { quizId, text, answer, options, image } = req.body;
    ...
    return res
      .status(201)
      .json({ message: "Question created and added to the quiz successfully", question: newQuestion });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error", error: error.message });
  }
